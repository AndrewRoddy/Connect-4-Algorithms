...
class Connect4:
    def __init__(self):
        ...
        self.screen_x, self.screen_y = 0, 0  # The current screen size
    ...
    def main_menu(self, screen):
        ...
        modes = [
            ["P_key", pygame.K_p, "pvp"],
            ["R_key", pygame.K_r, "random"],
        ...
        for i in range(len(modes)):
            self.image[modes[i][0]] = pygame.transform.scale(
                self.image[modes[i][0]], (self.size * 0.15, self.size * 0.15)
            )
            rect = self.image[modes[i][0]].get_rect(
                center=(
                    i * self.screen_x / 6.5 + self.screen_x / 9,
                    self.screen_y * 0.6,
                )
            )
            screen.blit(self.image[modes[i][0]], rect)
...
def main():
    ...
    menu = True
    frame_check = 0  # Updates the board once per second reducing lag
    while g.running:
        frame_check += 1
